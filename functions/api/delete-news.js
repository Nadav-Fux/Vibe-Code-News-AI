// Cloudflare Pages Function: POST /api/delete-news
// Body: { slug: "news-slug" }
// Auth: X-Editor-Secret header
...
export async function onRequestPost(context) {
  ...
  const res = await fetch(`${base}/mutate/${SANITY_DATASET}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SANITY_WRITE_TOKEN}` },
    body: JSON.stringify({
      mutations: [{ delete: { query: `*[_type == "news" && slug.current == "${slug}"]` } }],
    }),
  });
