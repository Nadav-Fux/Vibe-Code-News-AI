...
| `article.html` | Dedicated article page for Variation E. Defaults to a new-article template; use `?view=demo` for the filled demo article. |
...
- **Article Template:** `article.html` now opens a reusable longform RTL new-article template, while `article.html?view=demo` shows the filled demo version with TL;DR rail, sticky reading map, hero visual, pullquote, prompt/code card, comparison strip, and related articles.
